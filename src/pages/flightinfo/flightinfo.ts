...
constructor(public navCtrl: NavController, public navParams: NavParams, public formBuilder: FormBuilder, private authData: AuthProvider, private http: HttpClient, validator: ValidatorProvider) {
  ...
  this.pages=[{title: 'הזמנת נסיעה',component:FlightinfoPage},{title: 'נהג',component:DriverPage},{title: 'עריכת פרטים אישיים',component:ProfilePage},{title: 'התנתק',component:DisconnectPage}]
...
goToAddressPage() {
  this.flightStatus().then((data) => {
    ...
    if (this.checkexist)
      this.navCtrl.push(AddressPage, { flightNum: this.flightNum, date:this.flightNumForm.controls['flight_date'].value.formatted,time:data['time'] });
  });;
